...
anadir="data/freeMD_GLTPH_IFC"
chains=["M","N","O"]
def dwell_GLTPHIFC(lag=1):
    ...
    for chain in chains:
     for run in range(1,76):
        in1=tuple(open(anadir+"/run"+str(run)+"/states_"+chain+".xvg"))
        for i in range(0,len(in1)-1,lag):
            old=in1[i].split()[1]
            new=in1[i+1].split()[1]
...
print dwell_GLTPHIFC()
res=dwell_GLTPHIFC_bs()
print "---------Keq-----------"
z=np.zeros((1000,8,8))

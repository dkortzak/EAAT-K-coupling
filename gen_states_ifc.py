...
def assign_states3(laststate,distances):
        ...
        current_state=np.zeros(len(Kions))
        current_state_tmp=np.zeros(len(Kions))

        clearly_bound=np.zeros(shape=(3,len(Kions)),dtype=bool)
        clearly_unbound=np.zeros(shape=(3,len(Kions)),dtype=bool)
        clearly_bound[0,:] = distances[0,:]<3.6
        clearly_bound[1,:] = distances[1,:]<3.65
        clearly_bound[2,:] = distances[2,:]<3.65
        clearly_unbound[0,:] = distances[0,:]>11.5
        clearly_unbound[1,:] = distances[1,:]>8.5
        clearly_unbound[2,:] = distances[2,:]>9.5
...
for j in range(1,2):
  ...
  for chain in ["M","N","O"]:
    ...
    out=open("data/freeMD_GLTPH_IFC/run"+str(j)+"/states_"+chain+".xvg","w")
